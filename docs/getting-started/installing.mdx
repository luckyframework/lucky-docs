...
export const CustomField = ({ value }) => {
    ...
    return <>{customFields[value] || `No value found: ${value}`}</>
}
...
Lucky supports Crystal <code>&ge; <CustomField value="minCrystalVersion" /></code>, <code>&le; <CustomField value="maxCrystalVersion" /></code>
...
Should return between <code><CustomField value="minCrystalVersion" /></code> and <code><CustomField value="maxCrystalVersion" /></code>
...
<Tabs>
    ...
    <TabItem value="fedora" label="Fedora (28)">
...
This should return <code><CustomField value="luckyCliVersion" /></code>
...
<CodeBlock language="bash">
    {"git checkout "}<CustomField value="luckyCliTag" />
</CodeBlock>
...
Lucky uses Chromedriver for [Testing HTML](../testing/html-and-interactivity.md).
...
> [Flow](../testing/html-and-interactivity.md) guides for customization options.
